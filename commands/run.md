---
description: 按计划执行任务。逐个实现与验收，失败自动调试重试（不提交，提交交给 /br-ship）。
argument-hint: [可选：指定从哪个任务开始，如 "从 task-003 开始"]
---

# /run — 按计划执行

从实现计划中逐个执行任务，每个任务走"实现→验收"循环。**注意：/run 只负责把代码写出来并通过验收，不提交（commit）。** 提交是对外、需审查后才该做的动作，交给后续 `/br-review`（审查）和 `/br-ship`（提交+推送）。把 commit 嵌在执行循环里会让审查沦为事后擦屁股——审查的意义是"提交前的门禁"，代码都进 git 历史了才审查就晚了。
...
验收通过后，**只更新 state，不执行 git commit。** 代码改动保留在工作区，等所有任务完成、通过后续 `/br-review` 审查后，再由 `/br-ship` 统一提交。
...
- **级联调用**（路径 A，被 br-full-dev 调用）：输出总结后，**直接将控制权交还给父工作流**，继续执行后续审查与发布阶段。**改动全部保留在工作区，未提交**——提交动作交给阶段 5 的 `/br-ship`，且需用户确认。
