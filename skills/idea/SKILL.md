...
- 若无活跃 run（state.json 不存在或 `run.status !== "running"`）→ 视为入口，覆盖式初始化：`run.command: "idea"`、`run.path: "step"`、`phase.current: "explore"`、`phase.label: "需求分流"`
- 若已有活跃 run（被 `/br-full-dev` 编排调用）→ 不覆盖 run，只追加 `auto_decisions`（路由判断结果）
- 分流判定后写入：`auto_decisions` += {phase: "explore", decision: "路由到 br-office-hours/br-brainstorming", reason, auto}
